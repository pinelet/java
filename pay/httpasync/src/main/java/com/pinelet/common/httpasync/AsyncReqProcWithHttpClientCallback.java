 ...
 */
public class AsyncReqProcWithHttpClientCallback implements AsyncListener,
	...
	public AsyncReqProcWithHttpClientCallback(AsyncContext ac,
	...
	public void completed(HttpResponse response) {
		...
		if (null != ac) {
			...
			try {
				...
				HttpClientCallbackResult result = new HttpClientCallbackResult(
						is, null);
				...
				HttpClientCallbackResult result = new HttpClientCallbackResult(
						null, null);
				HttpAsyncException exp = new HttpAsyncException(
						HttpAsyncException.ExceptionEvent.COMPLELE_ERROR, e);
				result.setRetCode(getStatusCode());
	...
	public void failed(Exception ex) {
		...
		try {
			HttpClientCallbackResult result = new HttpClientCallbackResult(
					null, null);
			HttpAsyncException exp = new HttpAsyncException(
					HttpAsyncException.ExceptionEvent.REPLY_ERROR, ex);
			result.setRetCode(getStatusCode());
			result.setException(exp);
			callback.failed(result);
		} catch (RuntimeException e) {
	...
	public void onError(AsyncEvent arg0) throws IOException {
		...
		try {
			...
			HttpClientCallbackResult result = new HttpClientCallbackResult(
					null, os);
			HttpAsyncException exp = new HttpAsyncException(
					HttpAsyncException.ExceptionEvent.REQASYNC_ERROR,
					arg0.getThrowable());
			result.setRetCode(getStatusCode());
			result.setException(exp);
			callback.failed(result);
	...
	public void onTimeout(AsyncEvent arg0) throws IOException {
		...
		try {
			...
			HttpClientCallbackResult result = new HttpClientCallbackResult(
					null, os);
			HttpAsyncException exp = new HttpAsyncException(
					HttpAsyncException.ExceptionEvent.REQASYNC_TIMEOUT,
					arg0.getThrowable());
			result.setRetCode(getStatusCode());
			result.setException(exp);
			callback.failed(result);
